    ...
    <AnimatePresence>
      {show ? (
        <motion.div
          ...
          className={clsx(
            "fixed right-0 left-0 z-50 pointer-events-none px-5vw",
            {
        ...
        >
          ...
          >
            <div className="relative p-8 pr-14 max-w-xl bg-white rounded-lg shadow-md pointer-events-auto bg-inverse text-inverse">
              {typeof controlledVisible === "undefined" ? (
                <button
                  aria-label="dismiss message"
                  onClick={() => setIsVisible(false)}
                  className="absolute top-8 right-4 rotate-45 text-secondary hover:text-inverse focus:text-inverse"
                >
                  <PlusIcon />
                </button>
              ) : null}
